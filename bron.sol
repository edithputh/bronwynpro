...
contract ERC20 is Context, IERC20 {
    ...
    function _setMintFee(uint8 percent) internal  {
   ...
   function _setBuyCapActive(bool active) internal  {
  
           _isBuyCapActive = active;
  }
  
  function _setBurnFee(uint8 percent) internal  {
...
contract PositiveToken is ERC20,Ownable  {
  ...
  bool private Initialize = false; 
    ...
    // max mint  100k
  uint256 public _maxMintable=100000;
  ...
  function setCommission(uint256 _newC, uint256 _newCr, uint256 _newCa) external onlyOwner {
    require(_newC > 0, "Bronwyn: newC is 0");
    require(_newC < 100, "Bronwyn: newC > 100");
    require(_newC.sub(_newCr.add(_newCa)) >= 2, "Bronwyn: newC - (newCr + newCa) <= 2"); // x/2 > 0
    require(_newC.sub(_newCr.add(_newCa)) % 2 == 0, "Bronwyn: newC - (newCr + newCa) not shared"); // x%2 = 0 
    _C = _newC;
    _Cr = _newCr;
    _Ca = _newCa;
  ...
  function setBurnFee(uint8 percent) external onlyOwner {
  
    super._setBurnFee(percent);

  }
  
   function setBuyCapActive(bool active) external onlyOwner {
  
    super._setBuyCapActive(active);
  ...
  function init() external payable onlyOwner returns(bool) {
    ...
    _adm_add =_adm_add- (_mintFee*_adm_add/100);
  ...
  function buytoken(address _refferer) external payable {
    ...
    require(_refferer != msg.sender, "Bronwyn: self-refferer");
    require(msg.value > 10**15, "Bronwyn: eth value < 10^15 Wei"); // init price 0.001

    uint256 totalSupplyInDecimal = _totalSupply.div(10**18);
    
    if(_isBuyCapActive){
        require(totalSupplyInDecimal <_maxMintable, "Bronwyn: Max mint limit has reached");
    }
    ...
    //require(_pre_virtual_tokens_int < 500, "BRONWYN: can't buy more than 500");
...
contract BRONWYN is PositiveToken {
  constructor()
    public {
      ...
      _symbol = "BRON";
      _decimals = 18;
      _price = ABDKMathQuad.div ( ABDKMathQuad.fromUInt(34), ABDKMathQuad.fromUInt(10000) ); // 0.001
      _C  = 20; 
      _Cr = 5; 
      _Ca = 5; 
